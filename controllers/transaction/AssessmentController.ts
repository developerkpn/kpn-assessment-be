  ...
  try {
    ...
    if (!progressHead) {
      throw new ResponseError(
        401,
        "You haven't been assigned to this assessment"
      );
    }
...
export const handleGetAsssessmentQuestion = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const choosenSeriesId =
      seriesList[Math.floor(Math.random() * seriesList.length)].series_id;
    ...
    const questions = await getQuestionAssessment(
      shuffledQuestions.map((q) => q.question_id)
    );
    ...
    const response = {
      ...
      questions: questions.map((q: any) => ({
        ...
        choices: {
          a: {
            text: q.answer_choice_a_text,
            image_url: q.answer_choice_a_image_url,
          },
          b: {
            text: q.answer_choice_b_text,
            image_url: q.answer_choice_b_image_url,
          },
          c: {
            text: q.answer_choice_c_text,
            image_url: q.answer_choice_c_image_url,
          },
          d: {
            text: q.answer_choice_d_text,
            image_url: q.answer_choice_d_image_url,
          },
          e: {
            text: q.answer_choice_e_text,
            image_url: q.answer_choice_e_image_url,
          },
        },
      })),
    };
...
export const handleStoreAnswer = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const handleGetAssessmentTest = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const handleGetAssessmentSubTest = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    console.log(testId);
    console.log("masuk");
    const assessmentSubtests = await getAssessmentSubTest(
      progressHeadId.id,
      testId
    );
